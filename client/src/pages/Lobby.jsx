...
const Lobby = () => {
  ...
  useEffect(()=>{
    if (!socket){
     ...
     socket.emit("rtp_capabilities_req",{meetId:"1233456"},(response)=>{
      if (response.success) {
             ...
             <div className="flex  flex-col space-y-5  md:flex-row md:space-x-5 w-full mb-5">
               {/* for camera card */}
                ...
                {/* microphone  */}
                 ...
                 {/* join button */}
              <button className="font-bold  bg-green-500 h-12 w-40 rounded-lg shadow-lg   hover:cursor-pointer hover:bg-green-600 transition-colors duration-300">Join</button>
              </div>
